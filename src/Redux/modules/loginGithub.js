...
export function loginGithubRequest () {
  return dispatch => {
    ...
    return axios.get('/api/login/github')
      .then(res => {
      ...
      .catch(err => {
        ...
        if (err.response.data && err.response.data.message) {
          console.log('redux: loginGithub: invaild login credentials:', err.response.data.message)
          return dispatch(displayFlashMessage({ message: err.response.data.message, level: 'error' }))
        } else {
          console.error('redux: loginGithub: loginRequest failed:', err, err.response)
          return dispatch(displayFlashMessage({ message: 'An error occurred.', level: 'error' }))
        }
      })
  }
